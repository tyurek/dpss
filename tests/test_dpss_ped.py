...
from dpss.dpss_ped import DPSS_PED
...
async def test_dpss(test_router):
    ...
    secret, secret_hat = ZR.random(), ZR.random()
    poly = polynomials_over(ZR)
    phi, phi_hat = poly.random(d, secret), poly.random(d, secret_hat)
    shares = [phi(id+1) for id in old_committee['ids']]
    shares_hat = [phi_hat(id+1) for id in old_committee['ids']]
    comms = [g**(phi(0)) * h**(phi_hat(0))] + [g**(phi(id+1)) * h**(phi_hat(id+1)) for id in old_committee['ids']]
    dpss_tasks = [None] * n

    hbavss_list = [None] * len(ids)
    dpss_tasks = [None] * len(ids)
    for i in range(len(ids)):
        dpss = DPSS_PED(pks, sks[i], g, h, old_committee, new_committee, ids[i], G1, ZR, sends[ids[i]], recvs[ids[i]])
        hbavss_list[i] = dpss
        if ids[i] in old_committee['ids']:
            dpss_tasks[i] = asyncio.create_task(dpss.dpss(0, share=shares[i], share_hat = shares_hat[i], comms=comms))
        else:
