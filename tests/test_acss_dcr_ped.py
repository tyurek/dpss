...
async def test_acss_dcr(test_router):
    ...
    hbavss_list = [None] * n
    for i in range(n):
        hbavss = ACSS_DCR_PED(pks, sks[i], g, h, ids, t, d, ids[i], G1, ZR, sends[ids[i]], recvs[ids[i]])
        hbavss_list[i] = hbavss
        if ids[i] == dealer_id:
            avss_tasks[i] = asyncio.create_task(hbavss.avss(0, values=[(secret, secret_hat)]))
        else:
...
async def test_acss_dcr_external_dealer(test_router):
    ...
    dealeracss = ACSS_DCR_PED(pks, sks[i], g, h, ids, t, d, dealer_id, G1, ZR, sends[dealer_id], recvs[dealer_id])
    dealertask = asyncio.create_task(dealeracss.avss(0, values=[(secret, secret_hat)]))
