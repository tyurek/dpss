...
from dpss.acss_dcr_ped_batch import ACSS_DCR_PED_BATCH
...
async def test_acss_dcr(test_router):
    ...
    hbavss_list = [None] * n
    for i in range(n):
        hbavss = ACSS_DCR_PED_BATCH(pks, sks[i], g, h, ids, t, d, ids[i], G1, ZR, sends[ids[i]], recvs[ids[i]])
...
async def test_acss_dcr_external_dealer(test_router):
    ...
    dealeracss = ACSS_DCR_PED_BATCH(pks, sks[i], g, h, ids, t, d, dealer_id, G1, ZR, sends[dealer_id], recvs[dealer_id])
