...
class ACSS_DCR_PED:
    ...
    def _process_avss_msg(self, avss_id, dealer_id, rbc_msg):
        ...
        comms, encryptions, encryptions_hat, _ = loads(rbc_msg)
        share = self.ZR(self.private_key.raw_decrypt(encryptions[self.receiver_ids.index(self.my_id)]))
        share_hat = self.ZR(self.private_key.raw_decrypt(encryptions_hat[self.receiver_ids.index(self.my_id)]))
        self.output_queue.put_nowait((dealer_id, avss_id, [int(share)], [int(share_hat)], comms))
    ...
    def _get_dealer_msg(self, secret, secret_hat):
        ...
        return dumps([comms, encryptions, encryptions_hat, proofs])
    ...
    async def avss(self, avss_id, values=None, dealer_id=None):
        ...
        if self.my_id == dealer_id:
            ...
            secret, secret_hat = values[0][0], values[0][1]
            broadcast_msg = self._get_dealer_msg(secret, secret_hat)
        ...
        async def predicate_with_zero(_m):
            comms, encryptions, encryptions_hat, proofs = loads(_m)
