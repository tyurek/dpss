...
async def optqrbc(sid, pid, ids, f, leader, predicate, input, output, send, receive):
    """
    ...
    stripes = [None for _ in range(len(ids))]
    relative_idxs = {}
    i = 0
    for id in ids:
        relative_idxs[id] = i
    ...
    while True:  # main receive loop
        try:
            ...
            if msg[0] == RBCMsgType.PROPOSE and leader_hash is None:
                (_, leader_msg) = msg
                if sender != leader:
                    logger.debug(f"[{pid}] PROPOSE message from other than leader: {sender}")
                    continue
                ...
                if echo_counter[_digest] >= echo_threshold and not ready_sent:
                    ready_sent = True
                    logger.debug(f"[{pid}] in {sid} is broadcasting READY")
                    broadcast((RBCMsgType.READY, _digest))
            
            elif msg[0] == RBCMsgType.READY:
                (_, _digest) = msg
                if sender in ready_senders:
                    logger.debug(f"[{pid}] Redundant R")
                    continue
                ready_senders.add(sender)
                logger.debug(f"[{pid}] in {sid} has received ready messages from {ready_senders}")
                ...
                if ready_counter[_digest] == output_threshold:
                    ...
                    if _digest == leader_hash:
                        ...
                        logger.debug(f"[{pid}] in {sid} expected {_digest} but got {leader_hash} from the leader!!")
                        broadcast((RBCMsgType.ADD_TRIGGER, 0))
                        

            elif msg[0] == RBCMsgType.TERMINATE:
                if sender in terminate_senders:
                    logger.debug(f"[{sid}] Redundant TERMINATE from {pid}")
                    continue
                terminate_senders.add(sender)
                if len(terminate_senders) == len(ids):
                    return


            elif msg[0] == RBCMsgType.ADD_TRIGGER:
                if sender in add_trigger_senders:
                    logger.debug(f"[{pid}] Redundant ADD_TRIGGER")
                    continue
                add_trigger_senders.add(sender)
                if committed:
                    if stripes is None:
                        stripes = encode(k,n,m)
                    send(sender, (RBCMsgType.ADD_DISPERSE, stripes[relative_idxs[sender]], stripes[relative_idxs[pid]]))
                ...
                if sender in add_disperse_senders:
                    logger.debug("[{pid}] Redundant ADD_DISPERSE")
                    continue
                ...
                stripes[relative_idxs[sender]] = sender_stripe
                ...
                if sender in add_reconstruct_senders:
                    logger.debug("[{pid}] Redundant ADD_RECONSTRUCT")
                    continue
                add_reconstruct_senders.add(sender)
                stripes[relative_idxs[sender]] = stripe
                ...
                print(f"[{pid}] invalid message in {sid}: msg")
        except ValueError:
            break
