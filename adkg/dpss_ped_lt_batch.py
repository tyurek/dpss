...
from adkg.acss_ped_lt_batch import Hbacss0
...
class DPSS_PED_LT_BATCH:
    def __init__(self, public_keys, private_key, g, h, old_committee, new_committee, my_id, G1, ZR, send, recv, pc):
        #init
    ...
    async def dpss(self, dpss_id, values=None):
        ...
        acss = Hbacss0(self.public_keys, self.private_key, self.g, self.h, self.new_committee['ids'], self.new_committee['t'], self.my_id, self.G1, self.ZR, acsssend, acssrecv, self.poly_commit)
        #send commits (all to all)
        if self.my_id in self.old_committee['ids']:
            assert values is not None, "Missing required share"
            ...
            acss_oc = Hbacss0(self.public_keys, self.private_key, self.g, self.h, self.old_committee['ids'], self.old_committee['t'], self.my_id, self.G1, self.ZR, acsssend_oc, acssrecv_oc, self.poly_commit)
            ...
            def gen_aba_task(id):
                ...
                def broadcast(m):
                    ...
                    for id2 in self.old_committee['ids']:
                        asend(id2, m)
            ...
            async def acss_updater():
                while len(self.acss_outputs.keys()) < len(self.old_committee['ids']):
                    ...
                    shares_i, shares_i_hat, sender_i = entry[2], entry[3], entry[0]
                    print("player " + str(self.my_id)+ " got acss " + str(sender_i))
                    
                    self.acss_outputs[sender_i] = (shares_i, shares_i_hat)
            ...
            while len(ba_outputs.keys()) < len(self.old_committee['ids']):
                print(f"{self.my_id} awaiting aba outputs")
            ...
            global_randoms = []
            global_randoms_hat = []
            print("calculating global randoms")
            ...
            print("starting batch recconstruct")
            task = asyncio.create_task(batch_reconstruct_ped(True, self.old_committee['ids'], self.my_id, self.old_committee['t'], self.new_committee['degree'], self.g, self.h, br_output.put_nowait, self.ZR, self.G1, sendbr, recvbr, shares=blinded_shares, shares_hat=blinded_shares_hat))
            task.add_done_callback(print_exception_callback)
            
            blinded_shares, blinded_shares_hat = await br_output.get()
            print("sending br results")
            for id in self.new_committee['ids']:
                #todo: need more efficient method for batch-amortized
            ...
            self.output_queue.put_nowait((dpss_id, shares, shares_hat))
