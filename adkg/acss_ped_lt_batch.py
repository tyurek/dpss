...
from adkg.serializer import serialize as dumps, deserialize as loads
...
class Hbacss0:
    ...
    async def _process_avss_msg(self, avss_id, dealer_id, rbc_msg):
        ...
        if self.tagvars[tag]['all_shares_valid']:
            ...
            commitments = self.tagvars[tag]['commitments']
            #todo: bracha?
            self.output_queue.put_nowait((dealer_id, avss_id, self.tagvars[tag]['shares'], self.tagvars[tag]['shares_hat']))
            output = True
