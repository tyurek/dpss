...
from dpss.acss_dcr2 import ACSS_DCR
        ...
        if self.my_id in self.old_committee['ids']:
            ...
            def gen_aba_task(id):
                ...
                def broadcast(m):
                    for id2 in self.new_committee['ids']:
                        asend(id2, m)
                task = asyncio.create_task(tylerba(abatag, self.my_id, self.new_committee['ids'], self.new_committee['t'], None, ba_inputs[id].get, ba_outputs[id].put_nowait, broadcast, arecv))
            ...
            while True:
                ...
                if comms[idx_i+1] == c_i[0]:
                    ...
                    #shares.append((sender_i+1, share_i))
                    #comm_vecs.append((sender_i+1, c_i))
                    if len(self.acss_outputs) == self.new_committee['degree'] + 1:
                        for id in self.old_committee['ids']:
                            if id not in self.acss_outputs.keys():
                                ba_inputs[id].put_nowait(0)
                        print("player " + str(self.my_id)+ " gathering")
                        subset  = await asyncio.gather(*[ba_outputs[id].get() for id in self.old_committee['ids']])
                        print(subset)
                    if len(self.acss_outputs) > self.new_committee['degree']:
                        i, count = 0, 0
                        subsubset = set()
                        reset = False
                        while len(subsubset) <= self.new_committee['degree']:
                            if subset[i] == 1:
                                if self.old_committee['ids'][i] not in self.acss_outputs.keys():
                                    reset = True
                                    print("player " + str(self.my_id)+ " waiting for acss " + str(self.old_committee['ids'][i]))
                                    print("I did receive " + str(self.acss_outputs.keys()))
                                    break
