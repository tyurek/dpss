...
from dpss.acss_dcr_ped import ACSS_DCR_PED
...
class DPSS_PED:
    ...
    async def dpss(self, dpss_id, share=None, share_hat=None, comms=None):
        ...
        acss = ACSS_DCR_PED(self.public_keys, self.private_key, self.g, self.h, self.new_committee['ids'], self.new_committee['t'], self.new_committee['degree'], self.my_id, self.G1, self.ZR, acsssend, acssrecv)
        ...
        if self.my_id in self.old_committee['ids']:
            ...
            #subprotocol send...
            avss_task = asyncio.create_task(acss.avss(self.my_id, values=[(share, share_hat)], dealer_id=self.my_id))
            ...
            async def acss_updater():
                while len(self.acss_outputs.keys()) < len(self.old_committee['ids']):
                    ...
                    share_i, share_i_hat, c_i, sender_i = entry[2][0], entry[3][0], entry[4], entry[0]
                    ...
                    if comms[idx_i+1] == c_i[0]:
                        self.acss_outputs[sender_i] = (share_i, share_i_hat, c_i)
            ...
            shares_hat = [ (sender_i + 1, self.acss_outputs[sender_i][1]) for sender_i in subsubset]
            comm_vecs = [ (sender_i + 1, self.acss_outputs[sender_i][2]) for sender_i in subsubset]
            xlist = [share[0] for share in shares]
            lc = get_lagrange_coeffs(xlist, 0, self.ZR)
            print("lagrange worked")
            out_share, out_share_hat, out_comm = self.ZR(0), self.ZR(0), [self.G1.identity() for i in range(len(comm_vecs[0][1]))]
            for i in range(len(lc)):
                out_share += lc[i] * shares[i][1]
                out_share_hat += lc[i] * shares_hat[i][1]
            ...
            self.output_queue.put_nowait((dpss_id, [out_share], [out_share_hat], [out_comm]))
